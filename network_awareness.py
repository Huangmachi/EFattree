...
class NetworkAwareness(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		# Create bandwidth-sensitive datapath list
		self.bw_sensitive_dp_list = self.get_bw_sensitive_dp(CONF.fanout)
	...
	def get_topology(self, ev):
		...
		_graph = self.graph.subgraph(self.bw_sensitive_dp_list).copy()
		self.shortest_paths = self.all_k_shortest_paths(
			_graph, weight='weight', k=CONF.k_paths)
	...
	def get_bw_sensitive_dp(self, fanout):
		"""
			Get bandwidth-sensitive datapath list.
		"""
		dp_list = []
		Edge_List = [dp for dp in xrange(3001, 3001 + fanout ** 2 / 2)]
		Agg_List = []
		for i in xrange(1, fanout + 1):
			for j in xrange(fanout / 4 + 1, fanout / 2 + 1):
				num = fanout / 2 * (i - 1) + j
				PREFIX = "200"
				if num >= 10:
					PREFIX = "20"
				Agg_List.append(int(PREFIX + str(num)))
		Core_List = [dp for dp in xrange(1001 + fanout ** 2 / 8, 1001 + fanout ** 2 / 4)]
		dp_list = Edge_List + Agg_List + Core_List
		return dp_list
